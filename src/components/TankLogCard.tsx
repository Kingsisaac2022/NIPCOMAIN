...
import { VolumeLog } from '../types';
import Card from './Card';
import Button from './Button';
import SelectField from './SelectField';

interface TankLogCardProps {
  logs: VolumeLog[];
  stationName: string;
  tankName: string;
...
const TankLogCard: React.FC<TankLogCardProps> = ({ logs, stationName, tankName, stations }) => {
              ...
              >
                ...
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-text-secondary">
                    {log.previousVolume.toLocaleString()} L
                  </span>
                  <span className="text-text-secondary">→</span>
                  <span className="text-primary font-medium">
                    {log.newVolume.toLocaleString()} L
                  </span>
                </div>
                <div className="mt-2 text-sm text-text-secondary">
                  Change: {(log.newVolume - log.previousVolume).toLocaleString()} L
                </div>
              </div>
            ))}
          </div>
